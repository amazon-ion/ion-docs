...
Tegment's _encoding modules_--must have been defined or imported in the stream prior to
...
_Active modules_ are modules that are being used to encode the current stream segment.

In Ion 1.0, there is a single active module--the local symbol table.
In Ion 1.1, there can be one or more active modules.

Active modules are distinct from _available modules_—
modules which have not been referenced in an [encoding directive](#encoding-directives).

Modules can be defined in a stream without also being placed in the encoding context.
A module must be in the encoding context for its macros and symbols to be used to encode data.

...
It is legal for a local module to "shadow" a module in its parent scope by using the same name,
but it is not legal for a local module to use the same name as a module in the _same_ scope.

They inherit their spec version from the enclosing scope.

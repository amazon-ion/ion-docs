...
For illustrative purposes let's consider the module named `foo` that has a macro named `bar` at
offset 5 installed at the begining of the local macro table.
...
Macros also define a fixed set of zero or more parameters that they accept.  These parameters each have their own
cardinality which can be specified as _required_ (exactly one), _optional_ (zero or one), _zero or more_, and _one or
more_.

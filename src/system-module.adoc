...
The symbols and macros of the system module `$ion` is available everywhere within an Ion document,
with the version of that module being determined by the spec-version of each segment.

The specific system symbols are largely uninteresting to users since they don't affect the text
...
TODO Probably useful to allow some other Ion scalars (at least) to allow type conversion.
...
TODO if the coefficient were decimal, this could re-scale values. Useful?
...
TODO coerce an int or decimal to float?  Perhaps useful to use fixed-width ints to encode various float widths? This may not be useable to convert “IEEE bits” to float, since they would be converted to int before arriving here.
...
TODO Reconsider offset semantics, perhaps default should be UTC.

...
Within the spectrum of Ion 1.1's new encoding facilities, macros lie at the bottom, affecting the
serialized byte-stream.  The prior chapter explored macro definitions while ignoring the contexts
...
context-specific data maintained by an Ion implementation while encoding or decoding a document.
The Ion 1.0 encoding environment is effectively just the local symbol table.

The encoding environment is controlled by _directives_ embedded in the document at top-level.
These directives, also known as “system values”, are encoding artifacts and are not part of the
...
stable semantics over time.  Spec versions are expressed as Ion version markers, and a module can
only be used in segments encoded with that version or later.

// TODO The above may be too strict; use solely for symbols could be more relaxed.
...
IMPORTANT: Any existing Ion shared symbol table is a valid encoding module, albeit one that only
declares symbols and not macros.


=== Encoding Environment 1.1

In Ion 1.1, the encoding environment is enhanced to include:

  * The current Ion version, because a document may have segments using different Ion versions.
  * The _available modules_, a name->module mapping.
...
*$ion_1_0*
**$ion_symbol_table**::{
  imports: [{ name: "local", version: 1, max_id: 2 }]
}
----

NOTE:  Even ignoring Ion 1.1, this is how you would hoist local symbols into a
...
are effectively the `name`, `version` and `max_id` fields of the 1.0 `imports` struct.
...
Within a directive's `modules` field, both `*module*` and `*import*` clauses have the same
ultimate effect, declaring a name->module mapping for the encoding environment.
...
Just as in the 1.0 version, this allocates symbol IDs starting with ten from `shared1`, twenty
from `shared2`, then the two locally-defined symbols.

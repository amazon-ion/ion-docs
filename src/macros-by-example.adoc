...
This declaration—taken out of context to keep things simple—defines a public macro
named `pi`.  The `[]` is the macro’s signature, in this case a trivial one that declares no
parameters.  The macro therefore accepts no arguments and always returns a constant value.
...
Here we constrain the `asin` parameter to produce a string so the intent is clear and the Ion parser can emit an error sooner:
...
differentiates a null-field from an absent field (for example, `{a:null}` versus `{}`), macros
need to differentiate the absence of value from all other values in the Ion data model.
...
The special-case E-expression `(:)` is synonymous with `(:void)`
and exists solely as a more succinct argument expression for voidable parameters.
...
S-expression as a _delimiting container_ to group the applicable sub-expressions:
...
Note that the delimiter is required even for single values to avoid ambiguity.  Consider this macro:
...
If schema constraints are enforced through some mechanism outside the serializer/deserializer, the type tags are unnecessary and may add up to a non-trivial amount of wasted space.
...
Ion 1.1 tries to mitigate this overhead in the binary format by allowing macro parameters to have tagless types.  These are semantic subtypes of the concrete types,

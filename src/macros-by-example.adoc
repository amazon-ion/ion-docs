...
NOTE: While this doesn’t look like much of an improvement, the full string takes 38 to encode, but
the macro invocation can be encoded with as few as 12 bytes.
...
Some macros can accept an arbitrary number of arguments.  The `make_string` macro is one of those,
concatenating zero or more arguments into a single string:

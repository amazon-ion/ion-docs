...
In other words, a top-level syntax tree can be any general syntax tree, or an IVM.
...
delegate to stem:[\cal C_{\scriptsize import}] below.
...
to the name and version; it returns an optional *SharedSymtab* _s_.
...
Ion defines a standard serialization of SSTs, which we can easily compile:

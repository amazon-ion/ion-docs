...
// FIXME This footnote is not rendered properly: quotes and italic is handled.
...
To begin defining the expansion of a document, let's consider the state that
must be maintained while doing so.
...
stem:[\rho\!\downarrow_{\mbox{\scriptsize symbols}}] is the current symbol table.
...
[{denotation}]
++++
include::{modeltex}[tag=isIonSymbolTable]
\newline\newline
...
mode with the symbol table of the 1.0 system module.
...
easily compile{fn-sst-issue} into a shared module:
...
exact match, or has a larger version than the head, we return that.
...
The environment after an LST directive is as follows, where
stem:[{\cal C}_{\mbox{\scriptsize LST}}] denotes the compilation of an LST
directive into a new environment:
...
and the locally-declared symbols.
...
When a match is present, we adjust the number of symbols we return to align with
the `max_id` by using a helper function _adjust_length_ defined below.
Otherwise, we return padding consisting of _m_ copies of `$0`.

...
blob, and clob types, since they are "`atomic`" from the perspective of the
expansion process and thus have little effect on the remainder of the model.
...
*SymTok* denotes an expanded symbol token, either a Unicode string or `$0`.
...
null or a set of fields, and a field is a tuple with a symbol token and a value.
...
text.  We'll use the relation stem:[\cong] to notate this matching operation.
...
We'll use stem:[\odot] to represent this, here with an asterisk to indicate
that multiple values may result:
...
The (poorly-specified) intended connotation was that this is the data visible to
...
stem:[{\cal C}_{\mbox{\small LST}}] denotes the compilation of an LST
...
stem:[{\cal C}_{\mbox{\small\tt symbols}}] and
stem:[{\cal C}_{\mbox{\small\tt imports}}].
...
stem:[{\cal C}_{\mbox{\small symbol}}^{\ast}] but each

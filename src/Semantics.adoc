...
We recur on the tail of the field sequence (stem:[f^{\ast}\dagger 1]) and call
the results stem:[v^\ast].
...
symbol IDs, thereby producing the document's application-facing data.
...
The encoding environment is a tuple with three components.
  ...
  * The *catalog* is a collection of shared symbol tables provided to the
...
Expansion of <<stx-top,top-level syntax>> requires us to use a new technique
in our model, since it involves changes to the environment.
For example, the expansion of an
...
* All other top-level syntax is expanded using stem:[\cal E] and the
current environment stem:[\rho].
We check the resulting *Value* _v_ to see if it's an LST directive, in
...
* In the final *else*, the expanded _v_ is application data, so we pass it to
...
This may become more clear by considering the expansion of top-level streams:
...
when given no top-level syntax, the environment is unchanged and no data
is produced.
...
* That continuation is responsible for handling the tail of the stream
(stem:[\overline{\mbox{\it rv\/}^{\ast}}\dagger1]), which it expands via
...
This raises the question of where that initial continuation comes from.
We need a way to bootstrap expansion, and that happens via the user-facing
...
The initial environment is constructed from the given catalog, in Ion 1.0 mode,
...
stem:[\overline{{\cal E}^{\ast}}], so it discards the final environment and
passes the aggregated application data.
...
**SymTok**{asterisk}.
The input here is a sequence because struct lookup stem:[\odot] returns a
sequence of zero or one values, raising an error if the field is repeated.
...
Having extracted the field values from one import struct, we can produce a
sequence of **SymTok**s retrieved from the catalog.
...
to the name and version; it returns a sequence of at-most-one *SharedSymtab*
(stem:[s^{\ast}]).

If stem:[m^{\ast}] is non-empty, a valid `max_id` was given, and we can use
whatever we've found, even if we found nothing with the given name.
...
When there's no `max_id` we require an exact match: if stem:[s^{\ast}] has an
entry, and it has the requested version _v_, we return its symbols.
...
Having done that, we recursively walk the catalog, returning an at-most-one
sequence holding either an exact match for the name/version, or the highest
version, or nothing.

[{denotation}]
++++
include::{modeltex}[tag=best_match_SST]
++++

There's a lot of cases to account for here.
If the current entry (stem:[s_0]) has a different name, we return the recursive
result.
Otherwise, the name matches, so if the version matches or there's no match in the
tail (stem:[s^{\ast}_1]), we return the current entry.
Otherwise, if the best-match in the tail (stem:[s^{\ast}_1\downarrow_0]) is an

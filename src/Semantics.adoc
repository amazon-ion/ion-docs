...
To build a module from the sequence of (presumably) **String**s, we map a
helper function over the sequence of values in `symbols`:

[{denotation}]
++++
include::{modeltex}[tag=compile_SST_symbol]
++++

This validates that an entry is an actual string, returning its text,
or an absent entry (stem:[\langle \rangle]) when malformed.

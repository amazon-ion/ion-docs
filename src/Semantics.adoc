...
To similarly constrain their appearance in the model, we define them in a
separate domain for top-level raw values:
...
// FIXME LSTs are not relevant to TRValue, but explain why.
Ion 1.0 has two kinds of directives: Ion version markers (IVMs) and local symbol
tables (LSTs).
We model these top-level-only features through a top-level-only domain:
...
The _SymToks_for_IVM_ function produces the content of the system symbol table
for a given Ion version.  At present, the argument is ignored, but the future
Ion 1.1 symtab will go here.

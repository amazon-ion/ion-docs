...
disjoint sum of domains *D* and *E*, so when
stem:[\mbox{\bf Sum} = \mbox{\bf D} + \mbox{\bf E}] then every element of *Sum*
...
Cartesian product of **D** and **E**. These are tuples, effectively constrained
sequences containing first some
...
We'll eventually need to extract the elements inside a list, so let's

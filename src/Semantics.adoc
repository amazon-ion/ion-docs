...
That is, given a sequence of raw top-level values and a catalog (a set
...
IVM or an `$ion_symbol_table::` struct must produce a new environment, but no
application values.
...
The resulting **Value** _v_ is then checked to see if it's a symbol table
directive, in which case e use its content, downcast to **Struct**, to construct the new
environment.
...
gobbledygook appears inside an `$ion_symbol_table` directive, all
implementations will behave the same way.


=== System Symbol Tables

At the start of any Ion document, and following any IVM, the current symbol
table is set to the system symbol table of the relevant Ion version.
This is straightforward to define.

[{denotation}]
++++
include::{modeltex}[tag=system_symbols]
\newline\newline
include::{modeltex}[tag=LST_for_ivm]
\newline\newline
include::{modeltex}[tag=Environment_after_ivm]
++++

The __system_symbols__ function produces the content of the system symbol table
for a given Ion version.  At present, the argument is ignored, but the future
Ion 1.1 symtab will go here.  __LST_for_ivm__ simply lifts those tokens into
the **LocalSymtab** domain.

_Environment_after_ivm__ defines the environment that becomes current after the
IVM directive is encountered.  It retains the current environment's catalog,
but replaces its IVM and resets the symbol table to that of the Ion version.
...
[{denotation}]
++++
include::{modeltex}[tag=is_$ion_symbol_table]
++++

We can then turn the annotated struct into a LST:

[{denotation}]
++++
include::{modeltex}[tag=LST_for_$ion_symbol_table]
++++
...
When the directive is not null, we produce a LST by concatenating the imported
symbols and the locally-declared symbols.
...
[{denotation}]
++++
include::{modeltex}[tag=Environment_after_$ion_symbol_table]
++++

Here we see that the directive has the effect of retaining the current catalog
and Ion version, replacing only the local symtab.
More importantly, we see that an LST is fully expanded using the environment in
effect when it begins.
...
directive into a *LocalSymtab*. by delegating to helper functions
...
[{denotation}]
++++
include::{modeltex}[tag=is_$ion_shared_symbol_table]
\newline\newline
include::{modeltex}[tag=SST_for_$ion_shared_symbol_table]
++++
...
to test the size of local symbol tables.

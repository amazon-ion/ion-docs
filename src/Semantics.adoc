...
If _m_ is present, a valid `max_id` was given, so we call _bestModule_ (defined
below) to search the catalog for the best match to the name and version;
...
When there's no valid `max_id` we require an exact match; the _exactModule_
helper will signal an error if no exact match exists.

...
When we found an SST _s_, we compare its version to the one requested.
When they are equal, we have an exact-match; if `max_id` is well-formed we
must adjust the number of entries before returning it.
When the versions are not equal, `max_id` _must_ be well-formed, and we adjust
the length.
...
If the current entry (stem:[s_0]) has the requested name and version,
or the results from recursion on the tail (stem:[s^{\ast}_1]) include no matches,
we return the current entry.

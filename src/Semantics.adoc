...
:modeltex: ../build/tex/ion-model.tex
...
Given this model, we can derive two critical encoding features: the current
spec version, and the current symbol table.

[{denotation}]
++++
include::{modeltex}[tag=currentSpecVersion]
\newline\newline
include::{modeltex}[tag=currentSymtab]
++++

The latter denotes that a document's current symbol table is made up of the
symbols from the current system module, followed by the symbols declared in the
<<compile-lst,local symbol table>>.
Adding a compiled representation of `$0` to the front puts everything in the
right place to enable symbol expansion to index into the current symbol
table by SID, which we'll turn to now.
...
This requires a range check, raising an error if the SID exceeds the size of the
...
To complete the new module, we must compile the `symbols` list into a *SymTok*
sequence.
This takes some attention because of the rules regarding equivalence of absent
or malformed imported symbols.
These are not equivalent to `$0`, so we cannot return *Sym0*; we must instead
construct an *AbsentSym* that holds the module's name and symbol's index
within it.
The latter forces us to keep a counter _i_ during the recursive traversal:
...
Starting with the easy part, we compile an element of the `symbols` list
into a *SymTok*:

[{denotation}]
++++
include::{modeltex}[tag=compile_LST_symbol]
++++
...
it returns an optional *SharedModule* _mod_.
When a match is present, we extract its sequence of exported **SymTok**s, otherwise
we substitute the empty sequence.  We then use the upcoming _enforceMaxId_
to adjust the length per the `max_id`.
...
This time, we don't need to adjust the length of the module's *SymTok* sequence.

// TODO update text
The best-match case calls _enforceMaxId_ to ensure the import uses exactly
`max_id` **SymTok**s.
If the given stem:[t^{\ast}] is shorter than that, we (recursively) pad the end
with *AbsentSym*; if it's longer, we truncate to the right length.

[{denotation}]
++++
include::{modeltex}[tag=enforceMaxId]
++++

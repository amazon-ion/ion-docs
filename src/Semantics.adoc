...
necessary to get from there to here during expansion.  In particular, we need
to be able to match symbol tokens and symbol values by name, and to extract
field values from structs.
...
The atomic domains **NullNull**, **Boolean**, **Int**, and **String** pass
...
Compiling the `imports` elements is similar to _compile_symbols_ but each
